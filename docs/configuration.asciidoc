...
try {
    ...
    if ($previous instanceof 'Elasticsearch6\Common\Exceptions\MaxRetriesException') {
        echo "Max retries!";
    }
}
----
...
[source,php]
----
$connectionPool = '\Elasticsearch6\ConnectionPool\StaticNoPingConnectionPool';
$client = ClientBuilder::create()
...
[source,php]
----
$selector = '\Elasticsearch6\ConnectionPool\Selectors\StickyRoundRobinSelector';
$client = ClientBuilder::create()
...
[source,php]
----
$serializer = '\Elasticsearch6\Serializers\SmartSerializer';
$client = ClientBuilder::create()
