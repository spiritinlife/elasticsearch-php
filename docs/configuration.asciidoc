...
try {
    ...
    if ($previous instanceof 'Elasticsearch7\Common\Exceptions\MaxRetriesException') {
        echo "Max retries!";
    }
}
----
...
[source,php]
----
$connectionPool = '\Elasticsearch7\ConnectionPool\StaticNoPingConnectionPool';
$client = ClientBuilder::create()
...
[source,php]
----
$selector = '\Elasticsearch7\ConnectionPool\Selectors\StickyRoundRobinSelector';
$client = ClientBuilder::create()
...
[source,php]
----
$serializer = '\Elasticsearch7\Serializers\SmartSerializer';
$client = ClientBuilder::create()
