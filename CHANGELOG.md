...
- Reintroduced the optional parameter in `Elasticsearch6\Namespaces\IndicesNamespace::getAliases()`.
  This fixes the BC break introduced in 7.2.0 and 7.2.1.
  [#947](https://github.com/elastic/elasticsearch-php/pull/)

## Release 7.2.1

- Reintroduced `Elasticsearch6\Namespaces\IndicesNamespace::getAliases()` as proxy
...
- Fix type hinting for `setBody` in `Elasticsearch6\Endpoints\Ingest\Pipeline\Put`
...
- Simplified the logging methods `logRequestFail($request, $response, $exception)`
  and `logRequestSuccess($request, $response)` in `Elasticsearch6\Connections\Connection`
